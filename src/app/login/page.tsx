...
function LoginForm() {
  ...
  useEffect(() => {
    if (searchParams.get('verified') === 'true') {
      setSuccessMessage('VERIFIKASI BERHASIL! LANGSUNG LOGIN AJA YA');
    }
  }, [searchParams]);
  ...
  return (
    <div className="w-full max-w-md border border-zinc-800 bg-zinc-900/90 p-8 shadow-2xl backdrop-blur-md">
      {successMessage && (
        <div className="mb-6 border border-emerald-500/30 bg-emerald-500/10 p-3.5 text-xs font-bold uppercase tracking-wider text-emerald-400 text-center">
          {successMessage}
        </div>
      )}
